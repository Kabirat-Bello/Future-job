...
if page == "DASHBOARD":
 ...
 # salary
   ...
   # Question 4: How are job postings trending over time?
    ...
    # top_locations = location_salary.head(10)
    # top_locations['Salary'] = top_locations['Salary_Usd'].apply(lambda x: f"${x:,.0f}")
    # st.dataframe(
    #     top_locations[['Location', 'Salary']].style.background_gradient(
    #     cmap='Purples', subset=['Salary']
    # ),
    # use_container_width=True,
    # hide_index=True
    # )
    ...
    # # st.dataframe(
    # #     top_locations.style.background_gradient(cmap='Purples', subset=['Salary_Usd']),
    # #     use_container_width=True,
    # #     hide_index=True
    # # )
    ...
    # top_location = top_locations.iloc[0]
    # st.success(
    #     f"""The highest average salary is offered in **{top_location['Location']}**, 
    #     with an average pay of **${top_location['Salary_Usd']:,.0f}**. 
    #     This suggests that location plays a significant role in salary levels for future job roles."""
    # )
    ...
    #     y='Location',
    #     orientation='h',
    #     text=top_locations['Salary_Usd'].apply(lambda x: f"${x:,.0f}"),
    #     color='Salary_Usd',
    #     color_continuous_scale='Purples',
    #     title="Top Locations by Average Salary"
    # )
